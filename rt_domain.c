...
void __add_release(rt_domain_t* rt, struct task_struct *task)
{
	...
	list_add(&tsk_rt(task)->list, &rt->tobe_released);
	task->rt_param.domain = rt;
	if(!dp_Find_PID((task)->pid))
	{
		//THIHA KYAW
		// PID, WCET, Deadline, Priority1, dp_priority, PrPoint, tmrStatus, time_to_fired)
		add_dp_node((task)->pid, get_exec_cost(task), get_rt_relative_deadline(task), 
		get_priority(task), 6, 7, 0, get_release(task));
		calculate_Pri2_RM();
		calculate_ProPoint();
	...
	else
	{
		...
		tsk_rt(task)->task_params.priority = dp_get_Pri1((task)->pid);

		dp_set_tmrStatus((task)->pid, 0); // Status bit = 1 after add_release
