...
/*
static void dp_unBootPriority(struct task_struct* t)
{

	//fp_prio_remove(&pfp->ready_queue, t, priority_index(t));
	tsk_rt(t)->task_params.priority = dp_get_Pri1((t)->pid);
...
static void pfp_release_jobs(rt_domain_t* rt, struct bheap* tasks)
{
	...
	while (!bheap_empty(tasks)) {
		...
		if(dp_Find_PID((t)->pid))
		{
			dp_set_tmrStatus((t)->pid, 1); // Status bit = 1 after released
		
		}

		dp_set_time_Released((t)->pid, litmus_clock());

		TRACE_TASK(t, "released (part:%d prio:%d), %llu.\n",
			   get_partition(t), get_priority(t), litmus_clock());
...
//THIHA KYAW
static void dp_BootPriority(struct task_struct* t, pfp_domain_t *pfp)
{

	fp_prio_remove(&pfp->ready_queue, t, priority_index(t));
	tsk_rt(t)->task_params.priority = dp_get_Pri2((t)->pid);
	fp_prio_add(&pfp->ready_queue, t, priority_index(t));
...
static void job_completion(struct task_struct* t, int forced)
{
	...
	TRACE_TASK(t, "job_completion(): %llu.\n",litmus_clock());
	tsk_rt(t)->completed = 0;
...
static struct task_struct* pfp_schedule(struct task_struct * prev)
{
	...
	int out_of_time, sleep, preempt_status, np, exists, blocks, resched, migrate;
	int dp_task_PID;
	int bd_status;
	int promote_Current_tsk;
	struct task_struct *dp_t;


	bd_status = 0;
	promote_Current_tsk = 0;
	dp_t = NULL;

	//TRACE("Any Fired Task:  at %d, %llu, %llu\n",dp_get_PID_from_fireTask(), litmus_clock(), dp_get_time_to_Fire(dp_get_PID_from_fireTask()));
	...
	preempt_status     = !blocks && (migrate || fp_preemption_needed(&pfp->ready_queue, prev));

	/* If we need to preempt do so.
	 * The following checks set resched to 1 in case of special
	 * circumstances.
	 */
	resched = preempt_status;


	if(exists)
	{
		
		struct task_struct *trq = fp_prio_peek(&pfp->ready_queue);
		
		bd_status = dp_get_tmrStatus((pfp->scheduled)->pid);

		
		dp_task_PID  = dp_get_PID_from_fireTask();

		if(dp_task_PID>0 && (dp_get_tmrStatus(dp_task_PID)==3))
		{
			dp_t = find_task_by_vpid(dp_task_PID);
			...
			else if (dp_t && (dp_t != pfp->scheduled))
			{
				//Need to preempt
				/* Request a sys_exit_np() call if we would like to preempt but cannot.
				 * Multiple calls to request_exit_np() don't hurt.
			...
			else if (dp_t && (dp_t == pfp->scheduled))
			{
				...
				tsk_rt(dp_t)->task_params.priority = dp_get_Pri2((dp_t)->pid);
				promote_Current_tsk=1;
			}
			
			
			dp_set_tmrStatus((dp_t)->pid, 4);
			TRACE("Add to Ready Queue: PID: %d, tmrStatus: %d\n",(dp_t)->pid, dp_get_tmrStatus((dp_t)->pid) );
	 ...
	 * Multiple calls to request_exit_np() don't hurt.
	 */
	if (np && (out_of_time || preempt_status || sleep) && !promote_Current_tsk)
		request_exit_np(pfp->scheduled);
	 ...
	 * this.
	 */
	if (!np && (out_of_time || sleep) && !blocks && !migrate && !promote_Current_tsk) {
	 ...
	 */
	next = NULL;
	if ((!np || blocks) && (resched || !exists) && !promote_Current_tsk) {
		...
		if (next == prev) {
			...
			TRACE_TASK(next, "next==prev sleep=%d oot=%d np=%d preempt_status=%d migrate=%d "
				   "boost=%d empty=%d prio-idx=%u prio=%u\n",
				   sleep, out_of_time, np, preempt_status, migrate,
		...
		BUG_ON(preempt_status && next == prev);
		/* Similarly, if preempt is set, then next may not be NULL,
		 * unless it's a migration. */
		BUG_ON(preempt_status && !migrate && next == NULL);
	} else
		/* Only override Linux scheduler if we have a real-time task
		 * scheduled that needs to continue.
		 */
	if (exists && !promote_Current_tsk)
	{
		next = prev;
	}
	else if (exists && promote_Current_tsk)
	{
		next = dp_t;
	}
			
	if (next && !promote_Current_tsk) {

		TRACE_TASK(next, "scheduled at %llu\n", litmus_clock());

	} 
	else if (next && promote_Current_tsk)
	{
		TRACE_TASK(next, "scheduled at %llu with promoted priority\n", litmus_clock());
	}
	else {
